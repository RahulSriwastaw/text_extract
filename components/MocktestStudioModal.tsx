...
import { 
  ...
  Upload, Eye, Edit3, ChevronDown, ChevronUp, AlertCircle, 
            ...
            {items.length === 0 ? (
              ...
              <div className="space-y-4">
                {items.map((item, idx) => (
                  <div 
                    key={item.id}
                    className="glass-panel p-4 rounded-xl border border-white/[0.08] hover:border-white/[0.14] transition-all flex flex-col gap-3"
                  >
                      ...
                      <div className="flex flex-col gap-2.5 bg-white/[0.02] p-3 rounded-xl border border-white/[0.04]">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                          {(['option1_hi', 'option2_hi', 'option3_hi', 'option4_hi'] as const).map((key, optIdx) => {
                            ...
                            return (
                              <div key={key} className="flex items-center gap-1.5 bg-black/30 border border-white/[0.06] p-1.5 rounded-lg">
                                <span className="text-[10px] font-bold text-[#FF884D] w-4 text-center font-mono">({label})</span>
                                <input
                                  type="text"
                                  value={item[key] || ''}
